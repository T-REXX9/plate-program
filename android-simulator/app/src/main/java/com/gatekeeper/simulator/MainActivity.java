...
public final class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        Button clear = button("Vehicle leaves / clear loop"); controls.addView(clear); clear.setOnClickListener(v -> { state.loopActive = false; record("SENSOR loop=clear"); render(); });
        ...
        Button reset = button("Reset simulator"); controls.addView(reset); reset.setOnClickListener(v -> { state.reset(); timeline.setLength(0); record("RESET"); render(); });
    ...
    private void vehiclePresent() { if (state.gate != ControllerState.Gate.IDLE_CLOSED) { record("IGNORED loop present while " + state.gate); return; } state.loopActive = true; state.gate = ControllerState.Gate.WAITING_FOR_RFID; state.attemptUid = "sim-" + new SimpleDateFormat("yyyyMMdd'T'HHmmss'Z'", Locale.US).format(new Date()) + "-" + UUID.randomUUID().toString().substring(0,8); state.decision = "pending"; record("SENSOR loop=vehicle_present; attempt=" + state.attemptUid); render(); runRequest("CAPTURE", () -> client().capture(state.attemptUid), r -> { record("HTTP capture " + r.code + " in " + r.elapsedMs + " ms"); state.gate = ControllerState.Gate.RECOGNIZING; render(); pollAuthorization(); }); }
    private void sendRfid() { String value = rfid.getText().toString().trim(); if (value.isEmpty() || state.attemptUid.isEmpty()) { record("RFID ignored: enter RFID and start a vehicle attempt"); return; } runRequest("RFID", () -> client().rfid(state.attemptUid, value), r -> record("HTTP RFID " + r.code + " in " + r.elapsedMs + " ms; authorized=" + r.body.optBoolean("authorized", false))); }
    private void pollAuthorization() { runRequest("POLL", () -> client().accessResult(state.attemptUid), r -> { String status = r.body.optString("status", "unknown"); record("HTTP poll " + r.code + " in " + r.elapsedMs + " ms → " + status + timing(r.body)); if (status.equals("authorized")) authorize(r.body); else if (status.equals("denied")) deny(r.body); else main.postDelayed(this::pollAuthorization, 1000); }); }
    ...
    private void authorize(JSONObject body) { state.decision = "authorized"; state.plate = body.optString("plate", ""); resultLabel.setText("Server decision: AUTHORIZED · " + state.plate); record("AUTHORIZATION authorized; simulated barrier sequence begins"); if (automatic) openingSequence(); else { state.gate = ControllerState.Gate.OPENING; render(); } }
    private void deny(JSONObject body) { state.decision = "denied"; state.gate = ControllerState.Gate.DENIED; resultLabel.setText("Server decision: DENIED"); record("AUTHORIZATION denied; barrier remains closed"); render(); }
