...
public final class MainActivity extends Activity {
    ...
    private TextView gateLabel, sensorLabel, logLabel, resultLabel, identityLabel;
    private ImageView frameView;
    private EditText server, controller, key, rfid, pollInterval, rfidRetryInterval, authTimeout, openingDelay, openHold, closingDelay, networkDelay;
    private SecureStore secureStore;
    private boolean automatic = true;
    private boolean offline;
    private boolean attemptActive;
    private long attemptStartedAtNanos;
    ...
    private void buildUi() {
        ...
        rfidRetryInterval = input("RFID retry interval · default 1000", false); rfidRetryInterval.setText("1000"); root.addView(rfidRetryInterval);
        ...
        identityLabel = label("Server identity: not resolved", 14); root.addView(identityLabel);
        ...
        Button networkMode = button("Network simulation: ONLINE"); controls.addView(networkMode); networkMode.setOnClickListener(v -> { offline = !offline; networkMode.setText("Network simulation: " + (offline ? "OFFLINE" : "ONLINE")); record("MODE network=" + (offline ? "offline" : "online")); });
        ...
        Button reset = button("Reset simulator"); controls.addView(reset); reset.setOnClickListener(v -> { attemptActive = false; state.reset(); identityLabel.setText("Server identity: not resolved"); frameView.setImageDrawable(null); timeline.setLength(0); record("RESET"); render(); });
    ...
    private void vehiclePresent() { if (state.gate != ControllerState.Gate.IDLE_CLOSED) { record("IGNORED loop present while " + state.gate); return; } state.loopActive = true; state.gate = ControllerState.Gate.WAITING_FOR_RFID; state.attemptUid = "sim-" + new SimpleDateFormat("yyyyMMdd'T'HHmmss'Z'", Locale.US).format(new Date()) + "-" + UUID.randomUUID().toString().substring(0,8); state.decision = "pending"; attemptActive = true; attemptStartedAtNanos = System.nanoTime(); final String attempt = state.attemptUid; record("SENSOR loop=vehicle_present; attempt=" + attempt); render(); heartbeat(); runRequest("CAPTURE", () -> client().capture(attempt), r -> { record("HTTP capture " + r.code + " in " + r.elapsedMs + " ms; job=" + r.body.optString("job_id", "pending") + "; camera=" + r.body.optString("camera_id", "pending")); if (r.body.has("gate_id")) { String identity = r.body.optString("village_id", "") + " / " + r.body.optString("gate_id", ""); identityLabel.setText("Server identity: " + identity); record("SERVER resolved village=" + r.body.optString("village_id", "") + "; gate=" + r.body.optString("gate_id", "")); } if (r.body.has("annotated_image_available")) record("CAMERA annotated frame=" + r.body.optBoolean("annotated_image_available", false)); if (!attemptActive || !attempt.equals(state.attemptUid)) return; if (r.code < 200 || r.code >= 300) { record("CAPTURE failed; controller fails closed"); deny(new JSONObject()); return; } state.gate = ControllerState.Gate.RECOGNIZING; render(); pollAuthorization(attempt); }); }
    private void sendRfid() { String value = rfid.getText().toString().trim(); if (value.isEmpty() || !attemptActive || state.attemptUid.isEmpty()) { record("RFID ignored: enter RFID and start a vehicle attempt"); return; } sendRfid(value, state.attemptUid, 1); }
    private void sendRfid(String value, String attempt, int attemptNumber) { if (!attemptActive || !attempt.equals(state.attemptUid)) return; runRequest("RFID", () -> client().rfid(attempt, value), r -> { record("HTTP RFID " + r.code + " in " + r.elapsedMs + " ms; accepted=" + r.body.optBoolean("accepted", false) + "; authorized=" + r.body.optBoolean("authorized", false) + "; correlated=" + r.body.optBoolean("correlated", false) + "; duplicate=" + r.body.optBoolean("duplicate", false) + "; try=" + attemptNumber); if (r.code >= 500 && attemptNumber < 3) { long delay = millis(rfidRetryInterval, 1000); record("RFID transport retry scheduled in " + delay + " ms"); main.postDelayed(() -> sendRfid(value, attempt, attemptNumber + 1), delay); } }); }
    private void pollAuthorization() { pollAuthorization(state.attemptUid); }
    private void pollAuthorization(String attempt) { if (!attemptActive || attempt.isEmpty() || !attempt.equals(state.attemptUid)) return; long timeout = millis(authTimeout, AUTHORIZATION_TIMEOUT_MS); long elapsed = (System.nanoTime() - attemptStartedAtNanos) / 1000000L; if (elapsed >= timeout) { record("AUTHORIZATION timeout after " + timeout + " ms; controller fails closed"); deny(new JSONObject()); return; } runRequest("POLL", () -> client().accessResult(attempt), r -> { if (!attemptActive || !attempt.equals(state.attemptUid)) return; String status = r.body.optString("status", "unknown"); record("HTTP poll " + r.code + " in " + r.elapsedMs + " ms → " + status + timing(r.body)); if (r.body.has("plate")) record("RECOGNITION plate=" + r.body.optString("plate", "") + "; detector=" + r.body.opt("detector_confidence") + "; ocr=" + r.body.opt("ocr_confidence")); if (r.body.has("annotated_image_available")) { boolean available = r.body.optBoolean("annotated_image_available", false); record("CAMERA annotated frame=" + available); if (available) loadFrame(attempt); } if (r.code < 200 || r.code >= 300) deny(new JSONObject()); else if (status.equals("authorized")) authorize(r.body); else if (status.equals("denied")) deny(r.body); else main.postDelayed(() -> pollAuthorization(attempt), millis(pollInterval, 1000)); }); }
    ...
    private String timing(JSONObject body) { JSONObject t = body.optJSONObject("server_timing_ms"); return t == null ? "" : "; server queue=" + timingValue(t, "queue") + " processing=" + timingValue(t, "processing") + " total=" + timingValue(t, "total"); }
    private String timingValue(JSONObject timing, String name) { Object value = timing.opt(name); return value == null || value == JSONObject.NULL ? "pending" : value + " ms"; }
    private long attemptElapsedMs() { return (System.nanoTime() - attemptStartedAtNanos) / 1000000L; }
    private void authorize(JSONObject body) { attemptActive = false; state.decision = "authorized"; state.plate = body.optString("plate", ""); resultLabel.setText("Server decision: AUTHORIZED · " + state.plate); record("AUTHORIZATION authorized; capture-to-decision=" + attemptElapsedMs() + " ms; simulated barrier sequence begins"); if (automatic) openingSequence(); else { state.gate = ControllerState.Gate.OPENING; render(); } }
    private void deny(JSONObject body) { attemptActive = false; state.decision = "denied"; state.gate = ControllerState.Gate.DENIED; resultLabel.setText("Server decision: DENIED"); record("AUTHORIZATION denied; capture-to-decision=" + attemptElapsedMs() + " ms; barrier remains closed"); render(); }
    ...
    private void runRequest(String name, Request request, Complete complete) { long injected = millis(networkDelay, 0); record("REQUEST " + name + " started" + (injected > 0 ? " · injected delay " + injected + " ms" : "")); if (offline) { record("REQUEST " + name + " blocked by offline simulation"); state.gate = ControllerState.Gate.FAULT; attemptActive = false; render(); return; } network.execute(() -> { try { if (injected > 0) Thread.sleep(injected); ServerClient.Result r = request.run(); main.post(() -> complete.done(r)); } catch (Exception e) { main.post(() -> { record("REQUEST " + name + " failed: " + e.getClass().getSimpleName()); state.gate = ControllerState.Gate.FAULT; attemptActive = false; render(); }); } }); }
