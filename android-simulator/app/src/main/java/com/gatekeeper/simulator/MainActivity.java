...
public final class MainActivity extends Activity {
    ...
    private void sendRfid(String value, String attempt, int attemptNumber) { if (!attemptActive || !attempt.equals(state.attemptUid)) return; runRequest("RFID", () -> client().rfid(attempt, value), r -> { record("HTTP RFID " + r.code + " in " + r.elapsedMs + " ms; authorized=" + r.body.optBoolean("authorized", false) + "; try=" + attemptNumber); if (r.code >= 500 && attemptNumber < 3) { long delay = millis(rfidRetryInterval, 1000); record("RFID transport retry scheduled in " + delay + " ms"); main.postDelayed(() -> sendRfid(value, attempt, attemptNumber + 1), delay); } }); }
