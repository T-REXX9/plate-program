...
api /api/rfid-controller/status \
  ...
  --data-urlencode "credential_unrecognized=false" | pretty_json
...
if [[ -n "$rfid" ]]; then
  ...
  api /api/rfid-controller/recognitions \
    --data-urlencode "attempt_uid=$attempt_uid" \
    --data-urlencode "rfid=$rfid" | pretty_json
else
  echo
